...
import { FaPhoneAlt, FaRegFilePdf, FaChevronRight, FaShoppingCart } from 'react-icons/fa';
import { IndustryDetailData } from '@/types/templates.types';

export const IndustryDetailSidebar = ({ data, currentId }: { data: IndustryDetailData['sidebar'], currentId: string }) => {
  return (
    <div className="flex flex-col gap-8 w-full">
        ...
        {data.industriesLinks.map(link => {
          ...
          return (
            ...
            >
              ...
              <div className={`w-6 h-6 rounded-full flex items-center justify-center ${isActive ? 'bg-black/10' : 'bg-gray-800'}`}>
                <FaChevronRight className={`w-3 h-3 ${isActive ? 'text-black' : 'text-[var(--color-primary)]'}`} />
              </div>
            </Link>
          );
        })}
      ...
      <div className="bg-[#051024] rounded-2xl p-8 border border-gray-800/60 relative overflow-hidden group">
        ...
        <h3 className="text-3xl font-bold text-white mb-8 leading-tight">
          {data.contactCard.titlePart1} <span className="text-[var(--color-primary)]">{data.contactCard.titleHighlight}</span> {data.contactCard.titlePart2}
        </h3>
        ...
        <div className="flex flex-col items-center text-center gap-4 mb-8">
          ...
          <div>
            <p className="text-gray-400 text-sm mb-1">{data.contactCard.subtitle}</p>
            <p className="text-white font-bold text-xl">{data.contactCard.phone}</p>
          </div>
        </div>
