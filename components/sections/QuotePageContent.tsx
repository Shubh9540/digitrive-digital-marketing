...
import { FaPhoneAlt, FaEnvelope, FaMapMarkerAlt, FaBullseye, FaChartBar, FaLightbulb, FaUser, FaRegEnvelope, FaPhone, FaGlobe, FaRegFileAlt, FaPaperPlane, FaLock, FaChevronDown } from 'react-icons/fa';

const renderIcon = (iconName: string, className?: string) => {
  const props = { className: className || 'w-6 h-6 text-[var(--color-primary)]' };
  switch (iconName) {
    ...
    case 'FaBullseye': return <FaBullseye {...props} />;
    case 'FaChartBar': return <FaChartBar {...props} />;
    case 'FaLightbulb': return <FaLightbulb {...props} />;
    default: return <FaBullseye {...props} />;
  }
};
...
export const QuotePageContent = ({ data }: { data?: DigitalQuoteData }) => {
  ...
  return (
    <section className="bg-[#020914] pt-16 lg:pt-24 pb-12 text-white font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 mb-20">
          ...
          <div className="flex flex-col">
            ...
            <h2 className="text-4xl lg:text-5xl font-bold mb-6 leading-tight">
              ...
              <span className="text-[var(--color-primary)]">{data.leftCol.titlePart2}</span>
            </h2>
              ...
              {data.leftCol.features.map(feature => (
                <div key={feature.id} className="flex gap-5">
                  ...
                  <div className="flex flex-col justify-center">
                    ...
                    <p className="text-gray-400 text-sm leading-relaxed max-w-xs">{feature.text}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="relative rounded-2xl overflow-hidden shadow-[0_0_40px_rgba(0,229,255,0.1)]">
              <img src={data.leftCol.image} alt="Growth" className="w-full h-auto object-cover opacity-90" />
            </div>
          </div>
              ...
              <form className="flex flex-col gap-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="relative">
                    ...
                    <input type="text" placeholder={data.form.namePlaceholder} className="w-full bg-[#030b17] border border-gray-800/80 rounded-xl py-4 pl-12 pr-4 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-[var(--color-primary)] transition-colors" />
                  </div>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <FaRegEnvelope className="text-gray-400 w-4 h-4" />
                    </div>
                    <input type="email" placeholder={data.form.emailPlaceholder} className="w-full bg-[#030b17] border border-gray-800/80 rounded-xl py-4 pl-12 pr-4 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-[var(--color-primary)] transition-colors" />
                  </div>
                </div>
                ...
                <div className="relative">
                  ...
                  <input type="url" placeholder={data.form.websitePlaceholder} className="w-full bg-[#030b17] border border-gray-800/80 rounded-xl py-4 pl-12 pr-4 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-[var(--color-primary)] transition-colors" />
                </div>
                ...
                <div className="relative">
                  ...
                  <textarea placeholder={data.form.messagePlaceholder} rows={4} className="w-full bg-[#030b17] border border-gray-800/80 rounded-xl py-4 pl-12 pr-4 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-[var(--color-primary)] transition-colors resize-none"></textarea>
                </div>

                <button type="submit" className="w-full bg-[var(--color-primary)] text-black font-bold py-4 px-6 rounded-xl flex items-center justify-center gap-3 hover:bg-white transition-colors mt-2">
        ...
        {/* Bottom Contact Info */}
        <div className="w-full border-t border-gray-800/60 pt-10 mt-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {data.contact.map((item, index) => (
              <div key={item.id} className={`flex items-center gap-5 ${index !== data.contact.length - 1 ? 'md:border-r border-gray-800/60' : ''}`}>
                ...
                <div className="flex flex-col pr-4">
                  <span className="text-gray-400 text-xs font-medium mb-1">{item.title}</span>
                  <span className="text-white font-bold text-sm mb-1 whitespace-pre-line">{item.text}</span>
