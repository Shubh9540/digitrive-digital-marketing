...
import { FaBezierCurve, FaPenNib, FaChessKnight, FaCode, FaMobileAlt, FaSearch, FaBullhorn, FaArrowRight } from 'react-icons/fa';

const renderIcon = (iconName: string) => {
  switch (iconName) {
    case 'FaBezierCurve': return <FaBezierCurve className="w-5 h-5" />;
    case 'FaPenNib': return <FaPenNib className="w-5 h-5" />;
    case 'FaChessKnight': return <FaChessKnight className="w-5 h-5" />;
    case 'FaCode': return <FaCode className="w-5 h-5" />;
    case 'FaMobileAlt': return <FaMobileAlt className="w-5 h-5" />;
    case 'FaSearch': return <FaSearch className="w-5 h-5" />;
    case 'FaBullhorn': return <FaBullhorn className="w-5 h-5" />;
    default: return null;
  }
};
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center mb-8 lg:mb-10">
          ...
          <div className="lg:col-span-6 flex flex-col justify-center">
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white leading-tight mb-4">
              ...
              <span className="text-[var(--color-primary)]">{data.titlePart2}</span> Services
            </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.services.map((service) => (
            ...
            >
              <div className="flex items-start gap-6 mb-6">
                ...
                <div>
                  <h3 className="text-white font-bold text-xl mb-1 group-hover:text-[var(--color-primary)] transition-colors duration-300">
                    <span className="text-gray-500 font-normal mr-2">{service.number}</span>
                    {service.title}
                  </h3>
                </div>
              </div>
              ...
              <Link href={service.linkUrl} className="inline-flex items-center gap-2 text-[var(--color-primary)] text-sm font-medium hover:text-white transition-colors duration-300">
                {service.linkText}
