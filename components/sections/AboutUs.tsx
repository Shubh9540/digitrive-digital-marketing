...
const renderIcon = (iconName: string) => {
  switch (iconName) {
    case 'FaLightbulb': return <FaLightbulb className="w-5 h-5" />;
    case 'FaRocket': return <FaRocket className="w-5 h-5" />;
    case 'FaPen': return <FaPen className="w-5 h-5" />;
    case 'FaChartBar': return <FaChartBar className="w-5 h-5" />;
    default: return null;
  }
};
...
export const AboutUs = ({ data, hideButton }: { data?: DigitalAboutUsData; hideButton?: boolean }) => {
  ...
  return (
    <section className="py-16 lg:py-12 bg-[var(--color-bg-main)]">
          ...
          <div className="flex flex-col justify-between h-full py-4 lg:py-6 lg:col-span-7">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 mb-10">
              {data.features.map((feature, index) => {
                ...
                let borderClasses = "";
                if (isTopLeft) borderClasses = "border-b border-gray-800/60 py-6 sm:py-0 sm:pb-8 sm:pr-6 sm:border-r";
                if (isTopRight) borderClasses = "border-b border-gray-800/60 py-6 sm:py-0 sm:pb-8 sm:pl-6";
                if (isBottomLeft) borderClasses = "border-b border-gray-800/60 py-6 sm:py-0 sm:border-b-0 sm:pt-8 sm:pr-6 sm:border-r";
                if (isBottomRight) borderClasses = "pt-6 sm:pt-8 sm:pl-6";

                return (
                  <div key={feature.id} className={`flex items-start gap-5 ${borderClasses}`}>
