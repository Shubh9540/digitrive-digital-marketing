...
export const ProjectDetailContent = ({ data }: { data?: ProjectDetailData }) => {
  if (!data) return null;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 md:gap-4 py-8 border-b border-gray-800/60 mb-12">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-white font-bold text-lg md:text-xl">{data.meta.client}</span>
          </div>
          ...
          <div className="flex flex-col">
            ...
            <span className="text-white font-bold text-lg md:text-xl">{data.meta.category}</span>
          </div>
          ...
          <div className="flex flex-col">
            ...
            <span className="text-white font-bold text-lg md:text-xl">{data.meta.date}</span>
          </div>
          ...
          <div className="flex flex-col">
            ...
            <span className="text-white font-bold text-lg md:text-xl">{data.meta.location}</span>
          </div>
        ...
        <div className="flex flex-col gap-8 mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-2">{data.overview.title}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          <div className="relative h-64 md:h-80 lg:h-96 rounded-xl overflow-hidden">
            <Image src={data.middleImages.image1} alt="Project detail 1" fill className="object-cover" />
          </div>
          <div className="relative h-64 md:h-80 lg:h-96 rounded-xl overflow-hidden">
            <Image src={data.middleImages.image2} alt="Project detail 2" fill className="object-cover" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
          <div className="lg:col-span-5">
            <h2 className="text-4xl md:text-5xl lg:text-[48px] font-medium text-white leading-[1.2] tracking-tight">
              {data.facts.title.replace(' In ', '\nIn ').split('\n').map((line, i) => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {data.facts.list.map((fact) => (
                <div key={fact.id} className="flex items-center gap-3">
                  ...
                  <span className="text-gray-400 text-[15px]">{fact.text}</span>
                </div>
              ))}
            </div>
          </div>
